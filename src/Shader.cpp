...
const std::array<float, 3>& BasicShader::processFragment(const std::array<float, 3>& weights) 
{
    
    uint32_t color = TextureMapper::textureSample(this->diffuseTextureBuffer, this->diffuseTextureVertices, weights);
